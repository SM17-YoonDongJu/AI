...
- 테이블에 `masked_text`/`entities`처럼 다른 팀이 볼 필요 없는(또는 봐선 안 되는) 컬럼이 섞여 있으면 테이블 전체가 아니라 **컬럼 한정 `GRANT SELECT (col1, col2, ...)`**로 연다(`ai.ocr_results` → `app_owner`, `migrations/ai/011` 예시). `@Subselect`로 매핑하는 쪽이 Hibernate 식별자로 쓸 PK(`id`)를 컬럼 목록에서 빠뜨리기 쉬우니 챙긴다.
- 다른 워커가 이미 담당하는 크로스 스키마 쓰기(예: `core.reports` UPDATE)가 있으면, 같은 role이라 권한이 있어도 그 워커의 코드를 직접 재사용하지 않는다(별도 컨테이너일 수 있어 애초에 import가 안 될 수도 있다). 대신 그 워커가 이미 소비하는 메시지 계약으로 넘겨 기존 쓰기 경로를 태운다(`ocr_worker`가 `ReportJob.ocr_quality='needs_reupload'`를 발행해 `report_worker.mark_needs_reupload`가 쓰게 하는 패턴 — `sqs-worker-patterns` 참고).

...
--   blocked    전부 종결됐는데 정상 리포트를 낼 수 없다 → missing_doc_types에 사유를
--              남기고, 정상 리포트 대신 ocr_quality='needs_reupload'로 ReportJob을
--              발행한다(pipeline._judge_claim). 두 사유가 있고 겹칠 수 있다:
--              (a) 필수 문서 유형(보험증권·진단서)이 빠짐 — 업로드 자체는 필수라
--              "사용자가 안 올렸다"가 아니라 **"올렸는데 인식하지 못했다"**는 뜻이다
--              (분류 실패 등). (b) 업로드 수보다 성공한 문서 수가 적음(incomplete)
--              — 필수·비필수 안 가리고 하나 이상 결정적 실패했다는 뜻이다. missing_
--              doc_types는 (a)일 때만 채워진다.
--   published  청구 대표 ReportJob을 정상 발행했다.
--

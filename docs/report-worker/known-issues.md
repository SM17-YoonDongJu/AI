...
| # | 이슈 | 상태 | 영향 | 위치 |
|---|------|:---:|------|------|
| 1 | 좌우 신체부위 미구분 → 과소산정 | ✅ 해결(2026-08-09) | 지급률이 실제보다 낮게 | `agents.py` laterality + `disability_rules._region_key` |
| 2 | 검증 백스톱 무력화 → 환각 통과 | ✅ 해결(2026-08-09) | 없는 지급률이 verified로 | `agents._verify_rate_quote` + `tests/test_schedule_verify.py` |
| 3 | 신체부위 목록에 "외모" 누락 | ✅ 해결(2026-08-09) | 흉터·추상 장해 오분류 | 프롬프트 13부위로 확장 |
| 4 | `enrolled_at` NULL → 조용히 현행판 | 🟡 중간 | 구계약 버전 오매칭 | `agents.py:404` |
| 5 | 파생장해 규칙 미구현 | 🟡 중간 | 총칙 3-3항 미반영 | `disability_rules.py` |
| 6 | test_config가 로컬 `.env` 오염 | ✅ 해결(2026-08-09) | 로컬서 테스트 1건 상시 실패 | `_env_file=None` + `delenv` 격리 |
| 7 | ruff가 `scripts/`·`tempVectorDB/`서 실패 | 🟢 낮음 | CI는 `src tests` 스코프라 영향 없음 | (린트 설정) |

> 버그 1·2·3·6은 2026-08-09에 수정됐다. 아래 상세 서술은 당시 분석 기록으로 보존한다.
> 남은 항목: 4·5(제품 정책 결정 필요 — 별도 이슈 권장), 7(선택적 정리).

"""테스트 배터리 — 검색 다종 + e2e 다중 시나리오.
    ...
    PYTHONPATH=src python scripts/battery.py
...
from report_worker.rag.hybrid import search
...
SCENARIOS = [
    (
        ...
        1200000,
        "보험금이 적게 나온 것 같아요. 누락 특약 확인 원해요.",
    ),
    (
        "B_약관있음_골절",
        "현대해상",
        "무배당현대해상마음플러스(+)상해종합보험(Hi2601)",
        "disability",
        "좌측 요골 원위부 골절",
        "진단서\n진단명: 요골 골절 S52.5\n수술적 정복술, 입원 7일.",
        500000,
        "골절 진단비랑 수술비 받을 수 있나요?",
    ),
    (
        "C_약관없음_상품",
        "현대해상",
    ...
    (
        ...
        900000,
        "영구 후유장해가 남았는데 장해지급률이 어떻게 산정되나요?",
    ),
]
...
async def e2e_battery():
    ...
    for sc in SCENARIOS:
        ...
        try:
            ...
            branch = (
                "terms_parse(약관없음)"
                if any("runtime_parse_stub" in e for e in errs)
                else "직행(약관있음)"
            )
            ...
            print(
                f"  장해검토: {requires} | 장해분기실행: {bool(da)} | 지급률: {da.get('combined_rate')}% "
                f"| 신뢰도: {da.get('confidence')} | 근거수: {len(da.get('citations', []))}"
            )
            print(
                f"  draft 저장: {'OK' if saved else 'FAIL'} | issues: {len(final.get('issues', []))} | errors: {errs}"
            )
            # 라우팅 불변식: 후유장해 검토 필요면 반드시 장해 서브그래프가 실행돼 결과를 남겨야 한다
            assert (not requires) or bool(da), (
                f"{label}: requires_disability_review인데 장해 분기 미실행"
            )
        finally:
            await _cleanup(*ids)

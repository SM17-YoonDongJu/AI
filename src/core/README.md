...
| `contracts.py` | **Spring↔Python·워커 간 메시지 계약(단일 출처)**. SQS 큐 페이로드 + WebSocket 메시지 pydantic 모델 |
| `sqs/` | boto3 consumer/producer 래퍼 (롱폴링·역직렬화·ack(DeleteMessage)·poison 스킵) |
...
`config.py` → `contracts.py`(계약 합의) → `sqs`/`db`/`ai_client` 순으로 골격을 채운다. 컨벤션은 [.claude/CODE_CONVENTIONS.md](../../.claude/CODE_CONVENTIONS.md).

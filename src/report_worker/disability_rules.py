...
def combine_disability_rate(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Args:
        items: 각 원소는 최소 `body_region`, `rate`(%)를 갖고, 선택적으로 `temporary`(bool),
            `temporary_years`(number|None), `injury`/`category_label`(로그용)을 갖는다.
    ...
    for it in items:
        ...
        region = str(it.get("body_region") or "기타")
        normalized.append({**it, "effective_rate": eff, "region": region})

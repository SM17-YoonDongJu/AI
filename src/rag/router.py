...
CASE_CHUNKS) 2종이다. level(장해분류)·medical(수가·KCD)은 테이블 미존재로 향후 확장.
비신체보험(자동차·화재 등)은 범위 외로 안내한다.
"""

from dataclasses import dataclass

# 현재 검색 가능한 namespace. 소스 테이블로 부여되는 파생값과 동일 체계.
VALID_NAMESPACES: frozenset[str] = frozenset({"terms", "case"})
# 힌트가 없을 때 기본 검색 대상.
...
_NO_NAMESPACE_REASON = "검색 가능한 namespace가 없습니다(terms·case만 지원)."

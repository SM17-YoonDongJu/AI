...
보험·법률 문서를 처리하는 Python AI 엔진. **문서 OCR → Hybrid RAG 검색 → LangGraph 멀티에이전트 리포트 생성 → 가드레일 → 상담 챗봇**으로 구성된다. 파일 수신·JWT·S3 저장·Kafka 발행을 담당하는 **Spring Boot 게이트웨이는 별도 레포(범위 밖)**다.
...
──────────────────────────             ──────────────────────────────
업로드/JWT/S3/Kafka 발행      ──Kafka──▶  ocr_worker     (02, GPU 노드)
...
- **관리형**: DB = AWS RDS(PostgreSQL + pgvector), Kafka = MSK, Redis = ElastiCache

## 패키지 구조 & 담당

| 패키지 | 역할 | 노드 | 담당 | 상세 |
|---|---|---|---|---|
| `src/core` | 공용 인프라(설정·**계약**·kafka·db·ai_client·로깅) | — | _TBD_ | [README](src/core/README.md) |
...
| `src/ocr_worker` | OCR Kafka 워커 (02) | **GPU** | _TBD_ | [README](src/ocr_worker/README.md) |
...
Python 3.12 · **uv** · ruff · mypy · pytest · aiokafka · asyncpg + pgvector · FastAPI · structlog
...
# 3. 로컬 백킹 서비스 기동 (Kafka·Redis)
docker compose up -d

# 4. .env 작성 (DATABASE_URL=RDS, KAFKA_BOOTSTRAP_SERVERS, AI_BASE_URL 등)
```
...
cp .env.example .env          # 환경변수 작성 (DATABASE_URL=RDS, KAFKA_..., AI_BASE_URL 등)
docker compose up -d          # 백킹 서비스(Kafka·Redis) 기동
...
# 실행 (env로 RDS·MSK·ElastiCache·외부 Ollama/vLLM 주입)
...
1. **통합 경계 = `src/core/contracts.py`** (Kafka 토픽 페이로드 + WebSocket 메시지). **먼저 합의·고정**한 뒤 각 패키지를 병렬 개발한다. 계약이 어긋나면 워커 간 메시지가 깨진다. → 상세: **[.claude/docs/contracts.md](.claude/docs/contracts.md)**, 챗봇 이벤트는 **[.claude/docs/chatbot-events.md](.claude/docs/chatbot-events.md)**
...
- 백킹 서비스는 AWS 관리형(RDS·MSK·ElastiCache) + 외부 GPU 노드(Ollama/vLLM)를 **env로 주입**.
